...
def select_play_card(my_cards: list, player_card_counts: dict, before_card: dict, status: dict, order_dic: dict, wild_shuffle_flag: bool) -> dict:
    """
    出すカードを選出する
    ...
    if len(valid_card_list) > 0:
        ...
        if analyze_situation(my_cards, player_card_counts, wild_shuffle_flag) == "akan":
            tmp_list = decision_priority_0(valid_card_list)
...
def analyze_situation(my_cards: dict, player_card_counts: dict, wild_shuffle_flag: bool) -> str:
    """全体の手札の状況から戦況判断する関数"""
    min_cards_num = 100
    for v in player_card_counts.values():
        min_cards_num = min(v, min_cards_num)

    n = 1 if wild_shuffle_flag else 4
    # if wild_shuffle_flag:
    #     n = 1
    # else:
    #     n = 4
...
def decision_priority_0(cards: list) -> list:
    """
    手札の優先度を決める関数
    ...
    for card in cards:
        if "special" in card.keys():
            ...
            if card_special == "wild_shuffle":
                ...
                ans_list.append([card, (6, 1)])
        elif "number" in card.keys():
            ans_list.append([card, (7, card.get("number"))])
    # else:
    #     for card in cards:
    #         if "special" in card.keys():
    ...
    #                 ans_list.append([card,(3,1)])
    #         elif "number" in card.keys():
    #             ans_list.append([card,(2,card.get("number"))])
